...
import { clarity, pendingMessage, stageFor, threadGroup, threadLine, type ThreadGroup } from "../thread.ts";
...
export default function Threads({
  ...
  const groups: { key: ThreadGroup; title: string; items: ThoughtDraft[] }[] = (
    [
      { key: "active", title: "ACTIVE" },
      { key: "waiting", title: "WAITING" },
      { key: "done", title: "DONE" },
    ] as const
  ).map((g) => ({ ...g, items: visible.filter((t) => threadGroup(t, tasks) === g.key) }));
  const open = groups[0].items.length;
  return (
    <View style={s.root}>
      <View style={s.header}>
              ...
              {g.items.map((t) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Text style={[s.cardMeta, pending && !quiet && s.cardMetaLive]} numberOfLines={2}>
                      {threadLine(t, tasks)}
                    </Text>
                    {!quiet && (
                      <View style={s.track}>
...
const s = StyleSheet.create({
  ...
  empty: { padding: 18, borderRadius: 18, backgroundColor: C.card, gap: 6 },
