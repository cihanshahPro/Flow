...
func schema() throws -> GenerationSchema {
    ...
    return try GenerationSchema(root: DynamicGenerationSchema(name: "Shape", properties: [
        ...
        .init(name: "branches", description: "Other subjects in the person's words that are clearly separate from the thread's subject. Empty when everything belongs to one subject.", schema: .init(arrayOf: branch, minimumElements: 0, maximumElements: 3))
    ]), dependencies: [])
}
