...
test("Calendar lists watch-outs, then days with the phone's events and Flow's items; Your week is the same rows and ends with closure", async () => {
  ...
  assert.match(text, /Team sync · 11am|Team sync · 3pm|Team sync · /, "the phone's event as a tinted item");
  assert.match(text, /Follow up both lawyers · 16:00/);
...
test("the move sheet: day, time, due and takes as chips, project as a field, save carries the patch, delete is there", async () => {
  ...
  await tap("Tomorrow");
  await tap("Time: 10:00 AM");
  await tap("2:00 PM");
  await tap("Due: None");
  await tap("by Wed");
  await tap("Takes: 20 min");
  await tap("30 min");
