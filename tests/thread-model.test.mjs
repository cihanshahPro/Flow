...
const complete =
  "I need to finish the tax filing with my accountant before Friday because the deadline is strict.";
...
test("answering the missing question moves the thread toward ready", () => {
  ...
  const answer = suggestDraft("t3-a", "With my brother next Saturday, but only if it does not rain.");
  const next = appendPlanUpdate(first, answer);
  assert.equal(next.threadStatus, "ready");
  assert.equal(next.goalsReady, true);
  assert.deepEqual(next.missingPoints, []);
