...
function Desk() {
  ...
  const chooseCalendar: ChooseCalendar = (options, preferredId) =>
    new Promise((resolve) => {
      Alert.alert(
        "Choose a calendar",
        "New events go here. You can change this in Settings.",
        [
          ...options.map((option) => ({
            text:
              option.title + (option.id === preferredId ? " (default)" : ""),
  ...
  function resetLink(task: Task) {
    Alert.alert(
      "Reset calendar link?",
      "This does not delete the existing Apple event. Only reset after checking Calendar; saving again can create a duplicate.",
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: () =>
            void act(async () => {
              await resetCalendarLink(task.id);
              setMessage(
                "Calendar link reset. Existing Apple events were not deleted.",
              );
            }),
        },
      ],
        ...
        <View style={[s.screen, tab !== "Settings" && s.hidden]}>
          <ScrollView contentContainerStyle={s.content}>
            ...
            <View style={s.card}>
              ...
              <Text style={s.body}>
                On iPhone, allow Calendar access and choose a calendar once.
                Save creates or updates the linked event, including contacts,
                location, meeting link, and alert. Apple delivers alerts
                according to your Calendar notification and Focus settings.
                Changes made in Calendar do not sync back here; saving changed
                details here replaces the linked event's details. Android uses
                its calendar editor.
              </Text>
              {Platform.OS === "ios" && (
                <Button
                  title="Choose Apple calendar"
                  secondary
                  disabled={busy}
                  onPress={() =>
                    void act(async () => {
                      const id = await chooseAppleCalendar(
                        chooseCalendar,
                        true,
                      );
                      setMessage(
                        id
                          ? "Calendar chosen for new events."
                          : "Calendar selection canceled.",
                      );
                    })
                  }
                />
            ...
            <View style={s.card}>
              ...
              <Text style={s.small}>
                ...
                enabled. Calendar alerts are handled by your phone's Calendar
                app.
              </Text>
            </View>
          </ScrollView>
              ...
              >
                ...
                {editing && (
                  <>
                    ...
                    <Field
                      label="Time zone"
                      value={editing.timeZone || deviceTimeZone()}
                      onChangeText={(timeZone) =>
                        setEditing({ ...editing, timeZone })
                      }
                    ...
                    <View style={s.row}>
                      {REMINDERS.map((value) => (
                        <Pressable
                          ...
                          onPress={() =>
                            setEditing({ ...editing, reminderMinutes: value })
                          }
                        >
                          <Text
                            style={[
                    ...
                    <Field
                      ...
                      onChangeText={(contactName) =>
                        setEditing({ ...editing, contactName })
                      }
                    />
                    <Field
                    ...
                    <Field
                      ...
                      onChangeText={(location) =>
                        setEditing({ ...editing, location })
                      }
                    />
                    <Field
                      label="Meeting or website link (optional)"
                      value={editing.meetingUrl || ""}
                      onChangeText={(meetingUrl) =>
                        setEditing({ ...editing, meetingUrl })
                      }
                      placeholder="https://…"
                    />
                    <Field
